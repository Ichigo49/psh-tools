...
Function New-OSCWindowsTile
{
<#
 	.SYNOPSIS
        New-OSCWindowsTile is an advanced function which can be used to create a Windows 8 title to the Start menu.
    .DESCRIPTION
        New-OSCWindowsTile is an advanced function which can be used to create a Windows 8 title to the Start menu.
    .PARAMETER  <ShutdownTile>
		create a shutdown Windows 8 title to the Start menu.
	.PARAMETER  <RestartTile>
		create a restart Windows 8 title to the Start menu.
	.PARAMETER  <LogoffTile>
		create a logoff Windows 8 title to the Start menu.
    .EXAMPLE
        C:\PS> New-OSCWindowsTile
		
		This command shows how to create a shutdown, restart and logoff Windows 8 title to the Start menu.
    .EXAMPLE
        C:\PS> New-OSCWindowsTile -ShutdownTile
		
		This command shows how to create a shutdown Windows 8 title to the Start menu.
...
#>
    ...
    Param
    (
        [Parameter(Position=0,Mandatory,ParameterSetName="Shutdown")]
        [Alias('st','shutdown')][Switch]$ShutdownTile,
        [Parameter(Position=1,Mandatory,ParameterSetName="Restart")]
        [Alias('rt','restart')][Switch]$RestartTile,
        [Parameter(Position=2,Mandatory,ParameterSetName="Logoff")]
    ...
    Process
    {
        if($PSCmdlet.ShouldProcess("Windows Start menu","Create Windows tile"))
        {
            If($ShutdownTile)
            {
                CreateShutdownTile
